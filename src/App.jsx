...
const TABS = [
  ...
  { id: 'ask',       label: 'Ask Claude',     icon: SparkleIcon },
...
function applyFilters(deals, filters) {
  return deals.filter(deal => {
    ...
    if (filters.pipeline.length) {
      ...
      const untracke = filters.pipeline.includes('untracked') && !deal.tracked && !deal.passed
      if (!tracked && !passed && !untracke) return false
    }
    return true
  })
}
...
export default function App() {
  ...
  // Called from AI tab to apply filter and switch to Scout
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-[#0a0a0b]">
      ...
      <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
        {/* Tab bar */}
        <div className="flex-shrink-0 flex items-center gap-1 px-4 pt-3 pb-0 border-b border-[#2a2a2e] bg-[#0a0a0b]">
          {TABS.map(tab => {
            const Icon = tab.icon
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-3.5 py-2 text-[13px] font-medium rounded-t-lg border-b-2 transition-all -mb-px ${
                  isActive
                    ? 'text-[#f4f4f5] border-[#a855f7] bg-[#111113]'
        ...
        <div className="flex-1 min-h-0 overflow-hidden">
          ...
          {activeTab === 'scout' && (
            <div className="flex flex-col h-full">
              {/* Search + filter bar */}
              <div className="flex-shrink-0 flex items-center gap-3 px-4 py-3 border-b border-[#1e1e22] bg-[#111113]">
                ...
                {aiFilteredIds !== null && (
                  <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-purple-950 border border-purple-900">
                    <span className="w-1.5 h-1.5 rounded-full bg-purple-400" />
                    <span className="text-[12px] text-purple-300 font-medium">AI filter: {aiFilteredIds.length} match{aiFilteredIds.length !== 1 ? 'es' : ''}</span>
                    <button onClick={() => setAiFilteredIds(null)} className="text-purple-500 hover:text-purple-300 transition-colors ml-1">
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-[12px] font-medium transition-colors ${
                    Object.values(filters).some(a => a.length)
                      ...
                      : 'bg-[#18181b] border-[#2a2a2e] text-[#71717a] hover:text-[#a1a1aa] hover:border-[#3a3a3f]'
                  }`}
                >
                  ...
                  {Object.values(filters).reduce((n, a) => n + a.length, 0) > 0 && (
                    <span className="ml-0.5 text-blue-400">({Object.values(filters).reduce((n, a) => n + a.length, 0)})</span>
                  )}
                </button>

                <span className="text-[11px] text-[#52525b] ml-auto">{visibleDeals.length} of {totalCount} deals</span>
              </div>
          ...
          {/* ── Ask Claude Tab ── */}
          {activeTab === 'ask' && (
            <AIQueryTab deals={deals} onApplyFilter={handleApplyAiFilter} />
          )}
